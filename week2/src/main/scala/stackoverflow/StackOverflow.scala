...
class StackOverflow extends Serializable {
  ...
  def groupedPostings(postings: RDD[Posting]): RDD[(QID, Iterable[(Question, Answer)])] = {
    val rdd_1 = postings
      .filter(data => data.postingType == 1)
      .map(x => (x.id, x))

    val rdd_2 = postings
      .filter(data => data.postingType == 2 && data.parentId.isDefined)
      .map(x => (x.parentId.get, x))

    rdd_1.join(rdd_2).groupByKey()
  }
  ...
  def vectorPostings(scored: RDD[(Question, HighScore)]): RDD[(LangIndex, HighScore)] = {
    ...
    scored.map(x => (langSpread * firstLangInTag(x._1.tags, langs).get, x._2)).persist()
  }
