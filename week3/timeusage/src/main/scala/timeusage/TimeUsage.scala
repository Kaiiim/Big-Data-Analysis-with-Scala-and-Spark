...
object TimeUsage {
  ...
  /****************************************************************/
  /***************** Partie 2-> fonctions de classification *******/
  /****************************************************************/
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    val category = List(
      ...
      List("t1801","t1803","t1805"))

    def get_inf(x: String, i: Int, category: List[List[String]]): Boolean = {
      category(i).exists(test => x.startsWith(test))
    }
    (columnNames.filter(x => get_inf(x, 0, category)).map(column),
      columnNames.filter(x => get_inf(x, 1, category)).map(column),
      columnNames.filter(x => get_inf(x, 2, category)).filterNot(x => get_inf(x, 3, category)).map(column)
    )
  }
  ...
  def timeUsageSummary(
    ...
    def agg(x: List[Column], str: String): Column = x.reduce(_ + _).divide(60).as(str.toString)

    val primaryNeedsProjection: Column = agg(primaryNeedsColumns, str="primaryNeeds")
    val workProjection: Column = agg(workColumns, str="work")
    val otherProjection: Column = agg(otherColumns, str="other")
  ...
  /****************************************************************/
  /***************** Partie 3-> approches != pour les classer *****/
  /****************************************************************/
  ...
  def timeUsageGrouped(summed: DataFrame): DataFrame = {
    summed.groupBy($"working", $"sex", $"age")
          .agg(avg($"work").as("work"), avg($"primaryNeeds").as("primaryNeeds"), avg($"other").as("other"))
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String = {
    s"SELECT working, sex, age, ROUND(AVG(primaryNeeds),1) as primaryNeeds, ROUND(AVG(work),1) as work, ROUND(AVG(other),1) as other FROM summed GROUP BY working, sex, age ORDER BY working, sex, age "
  }
  ...
  def timeUsageGroupedTyped(summed: Dataset[TimeUsageRow]): Dataset[TimeUsageRow] = {
    ...
    summed
         .groupByKey(v => (v.working, v.sex, v.age))
         .agg(
