...
class WikipediaSuite extends FunSuite with BeforeAndAfterAll {
  ...
  test("'occurrencesOfLang' should work for (specific) RDD with 586 elem") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val res = occurrencesOfLang("Java", wikiRdd) == 586
    println(Console.BLUE + "occurenceofLang Java " +occurrencesOfLang("Java", wikiRdd))
    assert(res, "occurrencesOfLang given (specific) RDD with one element should equal to 1")
  }

  test("resultats de la liste") {
    assert(initializeWikipediaRanking(), " -- did you fill in all the values in WikipediaRanking (conf, sc, wikiRdd)?")
    import WikipediaRanking._
    val res = rankLangs(langs, wikiRdd) == List(("Scala",43), ("Ruby",120), ("Python",286), ("Perl",144), ("PHP",279), ("Objective-C",47), ("MATLAB",295), ("JavaScript",1692), ("Java",586), ("Haskell",54), ("Groovy",23), ("Clojure",26), ("CSS",372), ("C++",334), ("C#",705))
    println(Console.BLUE + "occurenceofLang Java " + res)
    //assert(res, "occurrencesOfLang given (specific) RDD with one element should equal to 1")
  }
  ...
  test("'makeIndex' creates a simple index with two entries") {
    ...
    //println(Console.BLUE+index.foreach(println))
  ...
  test("'rankLangsUsingIndex' should work for a simple RDD with three elements") {
    ...
    val rdd = sc.parallelize(articles)
    val index = makeIndex(langs, rdd)
    println(Console.BLUE+"index scala, java =>", index.foreach(println))
    val ranked = rankLangsUsingIndex(index)
    println(Console.BLUE+"ranked, list =>", ranked.foreach(println))
    val res = (ranked.head._1 == "Scala")
    assert(res)
  }
